...
function [] = lt_db_transfer_calls( copy_or_move )
...
if copy_or_move == 0 || copy_or_move == 1
    if ~exist([folder(1:dashes(end-1)) 'all_calls/'],'dir')
        mkdir([folder(1:dashes(end-1)) 'all_calls/'])
        mkdir([folder(1:dashes(end-1)) 'all_calls/' date_folder])
    elseif ~exist([folder(1:dashes(end-1)) 'all_calls/' date_folder],'dir')
        mkdir([folder(1:dashes(end-1)) 'all_calls/' date_folder])
    else
    end
end
