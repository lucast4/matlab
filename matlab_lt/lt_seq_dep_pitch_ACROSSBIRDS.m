...
if isfield(TrialStruct, 'FFalreadyFlippedLearnDir')
    disp('NOTE: FF all flipped. BUT for LMAN inactivated experiments that will not be true. See the lt_seq_dep_pitch_ACROSSBIRDS_TbyT_Extract code, where this occurs');
    pause;
end
...
if (0)
    ...
    save([savedir 'ParamsTrial.mat'], 'ParamsTrial');
end
if (0)
    % LOAD
    savedir = '/bluejay5/lucas/analyses/generalization/';
...
% -- params used for supplemental (longer duration learning)
% close all;
% RepeatsOnly=0;
% OnlyUseSylsInSylsUnique=1; %
% ExcludeSeqLearning=0;
...
if (0)
    ...
    % === defaults
    useSD = 0; % if 1, then plot SD instead of SEM on mean plots.
    plotRunningCV = 0;
    plotLMANalldata = 1;
    ignoreMUSC = 0
    
    % === use this code for plotting CV. 
    if (0)
    useSD = 1; % if 1, then plot SD instead of SEM on mean plots.
    plotRunningCV = 1;
    plotLMANalldata = 0;
    ignoreMUSC = 1;
    end
...
% ####################################### PLOT TIMECOURSES
close all; 
lt_seq_dep_pitch_ACROSSBIRDS_LMANbaseTC(DATSTRUCT);


% ####################################### PLOTS [PITCH AND ACOUSTIC]
close all;
lt_seq_dep_pitch_ACROSSBIRDS_LMANbasePlots(DATSTRUCT, SeqDepPitch_AcrossBirds_LMAN, ...
    PARAMS, same_type_thr, NullControl_splitday, useHandLabForSametype, ...
    onlyUseFirstExpt, recalcValues)
% ================== PUBLICATION VERSION
lt_seq_dep_pitch_ACROSSBIRDS_LMANbasePUB(DATSTRUCT);


% ####################################### PLOTS [CORRLEATIONS] [PUBLICATION
% VERSION INSIDE]
close all;
lt_seq_dep_pitch_ACROSSBIRDS_LMANbasePlotCor(DATSTRUCT, SeqDepPitch_AcrossBirds_LMAN, ...
    PARAMS, same_type_thr, NullControl_splitday, useHandLabForSametype, ...
    onlyUseFirstExpt, recalcValues)
...
%% [PREPROCESS] reextract MUSc data without normalizing for baseline musc effect?
