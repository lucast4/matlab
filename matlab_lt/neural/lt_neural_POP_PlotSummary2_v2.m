function lt_neural_POP_PlotSummary2_v2(MOTIFSTATS_pop, SummaryStruct, DATSTRUCT, ...
    plotRaw)
%% lt 1/22/19 - motified from v1 so that matches
% lt_neural_POP_PlotSummary_v2, in that here matches up motifs by their unique motif IDs
% (across expweriments_), sot hat experiemnts can have different motif strings or number o fmotifs
...
%     nummotifs = max(DATSTRUCT.Pairs.motifnum(DATSTRUCT.Pairs.birdID==i));
    nummotifs = max(DATSTRUCT.Pairs.motifID_glob(DATSTRUCT.Pairs.birdID==i));
    ...
    [~, motiflist_out] = lt_neural_QUICK_MotifID(bname);
    
%     birdname = MOTIFSTATS_pop.birds(i).birdname;
    for mm =1:nummotifs
        
        inds = find(DATSTRUCT.Pairs.birdID==i & DATSTRUCT.Pairs.motifID_glob==mm ...
...
if plotRaw==1
    ...
    nummotifs = max(DATSTRUCT.Pairs.motifID_glob);
