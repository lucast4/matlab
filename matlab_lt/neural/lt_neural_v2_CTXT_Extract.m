...
for i=1:numbirds
    ...
    for ii=1:numneurons
        ...
        % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% FOR SAM/MEL DATA, REMOVE SYLS THAT ARE PLACEHOLDERS
        if isfield(SummaryStruct.birds(i).neurons(ii), 'isRAsobermel');
            ...
            % ---- remove all BadSyls
            indstoremove = ismember(singlesyls', BadSyls);
%             unique(SongDat.AllLabels(~indstoremove))
            singlesyls(indstoremove) = [];
        end
        % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
