...
%% ======= ONE PLOT FOR EACH EXPT, PLOT RAW DATA. [target]
figcount=1;
subplotrows=4;
subplotcols=2;
fignums_alreadyused=[];
...
for i=1:length(allswitch_bnum)
    ...
    for ii=1:length(indsthis)
        ...
        % ---- get inds from xcov
       indsbase = find(OUTSTRUCT.indsbase{indsthisthis});
       indswn = find(OUTSTRUCT.indsWN{indsthisthis});
...
%% ======= ONE PLOT FOR EACH EXPT, PLOT RAW DATA. [DIFF TYPE]
figcount=1;
subplotrows=5;
subplotcols=3;
fignums_alreadyused=[];
...
for i=1:length(allswitch_bnum)
    ...
    indsthis = find(OUTSTRUCT.bnum==bb & OUTSTRUCT.enum==ee & OUTSTRUCT.switch==sw ...
        & OUTSTRUCT.istarg==0 & OUTSTRUCT.issame==0);
