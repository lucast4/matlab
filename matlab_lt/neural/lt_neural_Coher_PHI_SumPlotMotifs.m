function lt_neural_Coher_PHI_SumPlotMotifs(OUTSTRUCT, SwitchStruct,MOTIFSTATS_Compiled,PARAMS, indtoget_b_e_s)

%% lt 1/8/19 - plots phi during baseline trials, relative to posotion of motif
...
for i=1:length(indsgrp_switch_unique)
    ...
    for chanpair = indsgrp_chanpair_unique'
        ...
        phi = OUTSTRUCT.PhiScalar(indsthis);
        ...
        phi = phi(indsort);
        plot(motifID, phi, 'o-k');
        lt_plot_text(motifID(end)+0.3, phi(end), num2str(chnums), 'm', 9);
    end
    lt_plot_zeroline;
