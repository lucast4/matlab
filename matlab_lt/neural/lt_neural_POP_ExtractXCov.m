...
% 2: onset relative to token syl, offset relative to offset of last syl
% [only works appropriately with collected data for entire motif (osnet of
% token to offset of last syl)
...
Nmin = 7; % min number of trials, or else ignores this datapoint.
    ...
    for ii=1:numexpts
        ...
        for iii=1:numsets
            ...
            for mm = 1:nummotifs
                ...
                if isempty(MOTIFSTATS_pop.birds(i).exptnum(ii).DAT.setnum(iii).motif(mm).SegExtr_neurfakeID)
                    continue
                end
                
                segextract_for_trialdur = MOTIFSTATS_pop.birds(i).exptnum(ii).DAT.setnum(iii).motif(mm).SegExtr_neurfakeID(1).SegmentsExtract;
                
                if length(segextract_for_trialdur) < Nmin
                    continue
                end
                ...
                for nn=1:nneur
                    for nnn=nn+1:nneur
                        ...
                        if (0)
                            ...
                            maxtime = segextract_for_trialdur(1).motifsylOffsets(end)+motifpostdur;
                        end
                        
                        % === make sure all trials have one clust num (or
                        % no spikes)
                        assert(length(unique(cellfun(@unique, {dattmp1.SegmentsExtract(~cellfun(@isempty, {dattmp1.SegmentsExtract.spk_Clust})).spk_Clust})))==1, 'other clusts here ...');
                        ...
                        if xcov_dattotake_anchorpoints==1
                            % then both anchored to onset of token
                            motifendtime = motifpredur;
                        elseif xcov_dattotake_anchorpoints==2
                            % then end is anchored to end of motif
                            %  === 1) make sure all trials time warped, with
                            %  end aligned
                            assert(isfield(segextract_for_trialdur, 'LinTWSeg_OriginalMotifsylOffsets'), ...
                                'this only works if lin time warped, with all offsets identical');
                            tmp = [segextract_for_trialdur.motifsylOffsets];
                            tmp2 = length(segextract_for_trialdur(1).motifsylOffsets);
                            assert(all(diff(tmp(tmp2:tmp2:end))<0.002), 'not linearly time warped - i.e. trials dont all have same endpoint ...');
                            
                            % === 2) make the end time the common end of
                            % motif
                            motifendtime = segextract_for_trialdur(1).motifsylOffsets(end);
                        end
...
function [ccRealAll, ccPSTH, ccShiftAll, ccAuto1, ccAuto1Shift, ccAuto2, ...
    ...
    motifpredur, xcovwindmax, binsize_spk, plotSummary, plotThisCC, normmethod, motifendtime)
...
if (0)
    ...
    for t = 1:ntrials
        ...
        spkbin2shift = single(spkbin2shift);
        % --- limit to data in "premotor" window
        if ~isempty(xcov_dattotake)
            % shorten to desired window
            x = dattmp1.SegmentsExtract(t).spk_Binned_x;
