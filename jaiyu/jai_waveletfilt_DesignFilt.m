function jai_waveletfilt_DesignFilt(srate, fall)
%% lt, 11/15/18 - from Jai Yu, filter banks to calculate spectrogram

% samplingrate
% srate = 1500;
savedir = ['/bluejay5/lucas/analyses/neural/FILTER/JaiYu'];

% -- make it horizontal
if size(fall,2)<size(fall,1)
    fall = fall';
end
...
for f = fall
    ...
    %     figure
    %     plot(d)
    filterfile = sprintf([savedir '/cfcampfilt%s.mat'],num2str(f));
...
% {def/0: data is 1 epoch}
%   filtorder   = length of the filter in points {default
