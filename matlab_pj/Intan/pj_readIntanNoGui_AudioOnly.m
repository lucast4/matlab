...
% IF MULTIPLE ANALOG INPUTS, IS FINE, BUT WILL ONLY EXTRACT THE FIRST
...
%         board_dig_out_raw = zeros(1, num_board_dig_out_samples);
    ...
    % ============================ OLD METHOD [loops, about 50x slower]
    if (0)
        ...
        bytes_to_skip = 60*4 + 2*num_amplifier_channels*60 + 2*num_aux_input_channels*15 +...
            2*num_supply_voltage_channels*1 + 2*num_temp_sensor_channels*1 + ...
            2*60*(num_board_dig_in_channels>0) + 2*60*(num_board_dig_out_channels>0);
        
        % ------------- 1) get to start position for first datablock
        % In version 1.2, we moved from saving timestamps as unsigned
        % integeters to signed integers to accomidate negative (adjusted)
        %         % timestamps for pretrigger data.
        %         if ((data_file_main_version_number == 1 && data_file_secondary_version_number >= 2) ...
        ...
        % 2) ------ extract data
        N = 60*num_board_adc_channels;
%         tmp = fread(fid, Inf, [num2str(N) '*uint16'], bytes_to_skip)';
         tmp = fread(fid, [60, Inf], [num2str(N) '*uint16'], bytes_to_skip)';
        ...
        % 4) ------------------ SEPARATE CHANNELS
        if num_board_adc_channels>1
            numrows = size(tmp,1);
            rowsToGet = 1:num_board_adc_channels:numrows; % NOTE: to get all chans, just do this for each chan.
            tmp2 = tmp(rowsToGet, :);
            tmp2 = tmp2';
            board_adc_data = tmp2(:)';
        else
